...
class Img2Num(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(nn.Linear(1*28*28,200),nn.Sigmoid())
    ...
    def train(self):
        ...
        for epoch in range(100):#EPOCH = 1
            ...
            for step,(x,y) in enumerate(self.train_loader):
                ...
                b_y = Variable(y).cuda()
                output = self.forward(b_x)
                pred_y = torch.max(output, 1)[1].cuda().data.tolist()
            ...
            pred_y = torch.max(test_output, 1)[1].cuda().data.tolist()
            truey = test_y.tolist()
            for i in range(1000):
                point += (1 if pred_y[i] == truey[i] else 0)
            # print(test_output)
        ...
        plt.plot(validaterate,label="validate line")
        plt.ylabel('accurate_rate')
        plt.legend(loc='upper left')
        plt.xlabel('epoch')
        print(time.time()-time1,'s training time')
        plt.show()
...
test_data = torchvision.datasets.MNIST(root='./dataset/', train=False)
test_x = Variable(torch.unsqueeze(test_data.test_data, dim=1), volatile=True).type(torch.FloatTensor)[
         1000:2000].cuda() / 255.  # shape from (2000, 28, 28) to (2000, 1, 28, 28), value in range(0,1)
test_y = test_data.test_labels[1000:2000].cuda()
test_output = cnn.forward(test_x[:1000])
pred_y = torch.max(test_output, 1)[1].cuda().data.tolist()
truey = test_y.tolist()
for i in range(1000):
